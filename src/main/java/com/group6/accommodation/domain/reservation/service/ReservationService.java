...
public class ReservationService {
    ...
    @Transactional
    public ReservationResponseDto createReservation(Long userId, Long roomId, PostReservationRequestDto postReservationRequestDto) {
        UserEntity user = userRepository.getById(userId);
        RoomEntity room = roomRepository.getById(roomId);

        if(room.isOverCapacity(postReservationRequestDto.getHeadcount())){
        ...
        int price = room.getPayment(postReservationRequestDto, OVER_PRICE);


        ReservationEntity reservationEntity = ReservationEntity.builder()
            .room(room)
            .user(user)
            .headcount(postReservationRequestDto.getHeadcount())
            .startDate(postReservationRequestDto.getStartDate())
            .endDate(postReservationRequestDto.getEndDate())
            .price(price)
            .build();
